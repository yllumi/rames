 ...
 *
 * Menampilkan daftar domain (subdomain bawaan site, atau custom domain bila
...
class SslController
{
        ...
        foreach ($sites as $site) {
            ...
            if ($customDomain !== '') {
                $rows[] = [
                    ...
                    'domain' => $customDomain,
                    'is_custom' => true,
                    'ssl_status' => (string) ($site['custom_ssl_status'] ?? 'disabled'),
                    'ssl_message' => $site['custom_ssl_message'] ?? null,
                    'ssl_expires_at' => $site['custom_ssl_expires_at'] ?? null,
                    'ssl_error' => $site['custom_ssl_error'] ?? null,
                ];
            } else {
                $rows[] = [
                    'site' => $site,
                    ...
                    'ssl_message' => $site['ssl_message'] ?? null,
    ...
    /**
     * Aktifkan SSL untuk sebuah domain milik site.
     *
    ...
    public function enable(Request $request, string $id)
    {
        ...
        if ($domain !== $subdomain && $domain !== $customDomain) {
            flash_set('error', "Domain {$domain} bukan milik site ini.");
            return redirect('/ssl');
        }
        ...
        $status = (string) ($site[$slot . '_status'] ?? 'disabled');
     ...
     */
    private function spawnWorker(string $siteId, string $domain): bool
    {
        ...
        $command = [PHP_BINARY, base_path('cli/ssl.php'), $siteId, $domain];
        $descriptors = [
            0 => ['file', '/dev/null', 'r'],
            1 => ['file', $logFile, 'a'],
            2 => ['file', $logFile, 'a'],
